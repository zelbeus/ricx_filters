...
local Config = {
    Texts = {
        ...
        RemoveD = "Remove the selected filter",
        Filters = "Filters",
        Options = "Options",
    }
}

RegisterCommand("filters", function(source, args, raw)
    TriggerEvent("ricx_filters:open")
end, false)

function RemoveFilter()
    if lastfx then 
        AnimpostfxStop(lastfx)
        lastfx = nil 
    else
        AnimpostfxStopAll()
    end
...
RegisterNetEvent("ricx_filters:open", function(info)
    MenuData.CloseAll()
    local elements = {}
    menuOpen = true 
    elements[#elements+1] = {label = Config.Texts.Remove, value = "remove", desc = Config.Texts.RemoveD}
    for i,v in pairs(animpostfx) do 
        local label = v[1]
        if v[2] ~= "" then 
            label = v[2]
        end
        elements[#elements+1] = {label = label, value = v[1], desc = Config.Texts.Select}
    end 
    

    MenuData.Open('default', GetCurrentResourceName(), 'ricx_filters',{
         title    = Config.Texts.Filters,
